    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = f"Please specify a valid number and unit of time (hours/days) script by OWNER- @GODxAloneBOY"
        else:
            response = "Usage: /genkey <amount> <hours/days>"
    else:
        response = f"ONLY USE OWNER - @GODxAloneBOY"
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            del keys[key]
            save_keys()
            response = f"✅Key redeemed successfully! Access granted until: {users[user_id]} JOIN- https://t.me/godxcheat9bajkakakao"
        else:
            response = f"Invalid or expired key buy from OWNER- @GODxAloneBOY"
    else:
        response = f"Usage: /redeem <key> if you don't  have  buy from @GODxAloneBOY"
...
async def allusers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        if users:
            ...
            response = f"No data found OWNER- @GODxAloneBOY"
    else:
        response = f"ONLY OWNER CAN USE.OWNER- @GODxAloneBOY"
    await update.message.reply_text(response)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text('😈 ᗩTTᗩᑕK STᗩᖇTEᗪ 😈')
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    flooding_process = None
    await update.message.reply_text('❤️ ᗩTTᗩᑕK STOᑭEᗪ ❤️')
